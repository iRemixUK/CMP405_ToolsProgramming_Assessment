...
int Game::MousePicking()
{
	...
	for (int i = 0; i < m_displayList.size(); i++)
	{
		...
		for (int y = 0; y < m_displayList[i].m_model.get()->meshes.size(); y++)
		{
			...
			{
				// Check if picked distance is smaller than the closest distance
				// If so update the selected ID and closest distance
				if (pickedDistance <= closestDistance)
				{
					closestDistance = pickedDistance;
					highlightObject(selectedID, false);
					selectedID = i;
					highlightObject(selectedID, true);
					m_Camera.SetLookAt(m_displayList[i].m_position);
				}
			}
		}
	}
...
{
	DisplayObject& displayEntryRef = m_displayList.at(objectID);
	SceneObject& sceneEntryRef = SceneGraph->at(objectID);
...
DirectX::SimpleMath::Vector3 Game::CameraRightVector()
{
	return m_Camera.GetRightVector();
}

DirectX::SimpleMath::Vector3 Game::CameraForwardVector()
{
	return m_Camera.GetForwardVector();
}

float Game::GetCameraMovementSpeed()
{
	return m_Camera.GetCameraSpeed();
}

void Game::SetCameraMovementSpeed(float cameraMovementSpeed)
{
	m_Camera.SetCameraSpeed(cameraMovementSpeed);
}

float Game::GetCameraRotationRate()
{
	return m_Camera.GetCamRotationRate();
}

void Game::SetCameraRotationRate(float cameraRotationRate)
{
	m_Camera.SetCamRotationRate(cameraRotationRate);
}

void Game::highlightObject(int objectID, bool highlight)
{
	if (objectID > 0)
	{
		m_displayList.at(objectID).highlight(highlight);
	}
}
#pragma endregion
